...
class pixyTuning(object):
    ...
    # output        - "calibrationCurve_X.csv" with target tracking error in ([deg]) and camera angle in ([deg])
    #                 at each frame time
    # fVideo        - frequency of stimulus in MATLAB generated video file (>=0)
    def measure(self, fVideo: float):
    ...
    def updateServo(self):
        ...
        pixelError = self.cam.newBlocks[0].m_x - self.cam.pixyCenterX
        # error converted to angle
        visAngularCorrection = -(pixelError/self.cam.pixyMaxX*self.cam.pixyX_FoV)
        # error relative to pixycam angle
        visTargetAngle = self.bot.servo.lastPosition + self.bot.gimbal.update(visAngularCorrection)
        # set camera angle to calculated target angle
